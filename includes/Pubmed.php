...
class NQ_Pubmed
{
	...
	public $cache_dir = DB_CACHEDIR; // Directory where cached results will be saved
	public $cache_life = STORE_DATA_TIME; // Caching time, in seconds
	public $cache_file_hash = ''; // Sets to the md5 hash of the search term
	public $only_PMIDs = false; //return only pubmed IDs (false) or all data on a PMID (true)?
	public $term = '';
	public $db = 'pubmed';
	public $retmode = 'xml';
	public $exact_match = true; // Exact match narrows the search results by wrapping in quotes
	
	public $sleep = 0.35; // How long to wait between each search - for limits see http://www.ncbi.nlm.nih.gov/books/NBK25497/
	...
	public function is_cached($term)
	{
...
//		$data['count'] = $this->count;
	...
	public static function proxy_simplexml_load_file($url)
	{
		...
		if (isset(self::$proxy_name) && !empty(self::$proxy_name)) {
			...
			ini_set('user_agent', DOMAIN_NAME);
	...
	public function cache_results($term, $results)
	{
		...
		if (!$fh)
			die('Unable to write cache file to cache directory.');
