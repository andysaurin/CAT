...
class drosoCAT extends NQ_Auth_No
{
	public $class_title = "drosoCAT";
	...
	public function __construct()
	{
		...
		$this->prefs->chrom_sizes_bed = SYSTEM_DATA_ROOT . "/chromSizes_BED/dm3.bed";
	...
	private function add_encode_files() {
		...
		//add the modENCODE bed files to the files list
	...
	private function process_data($data=null, $files=null) {
		...
		*/
		...
		$this->prefs->increase_sensitivity = ( $data['increase_sensitivity'] == 'hsp' ? true : false );
		$this->prefs->zscore = ( $data['increase_sensitivity'] == 'zscore' ? true : false );
					...
					foreach( $results as $result ) {
						...
						if ( isset($this->collections[$collection_id]) ) {
							...
							if ( $this->prefs->zscore === false )
								...
								$score = z_score($result->observed, $result->expected, $result->stddev);
...
?>';
		...
		**/
		...
		if ( isset($no_pdf) ) {
			$success .= "<h4>You can download the results as a .tar file using <a href=\"/downloads/{$this->uniq_id}/".CAT_TYPE."_results.tar\" target=\"_blank\">this link</a>.</h4>";
//			$success .= "<h4>or as a .zip file using <a href=\"/downloads/{$this->uniq_id}/".CAT_TYPE."_results.zip\" target=\"_blank\">this link</a>.</h4>";
		} else {
			...
			$success .= "<h4>and download the results as a .tar file using <a href=\"/downloads/{$this->uniq_id}/".CAT_TYPE."_results.tar\" target=\"_blank\">this link</a>.</h4>";
//			$success .= "<h4>or as a .zip file using <a href=\"/downloads/{$this->uniq_id}/".CAT_TYPE."_results.zip\" target=\"_blank\">this link</a>.</h4>";
	...
	private function validate_form($data=null, $files=null) {
		...
		*/
		...
		if ( $data['omitHOT'] && ( !is_file(SYSTEM_DATA_ROOT . '/HOT_regions.dm3.bed') || !is_readable(SYSTEM_DATA_ROOT . '/HOT_regions.dm3.bed') ) )
			$errors[] = "Installation error: The file data/HOT_regions.dm3.bed is missing or is not readable.";
...
*/
	...
	*/
				...
				if ( $this->copy_and_sort($tmpfile, $sorted_file, $orig_fname) != false ) {
					...
					$command = "awk '{print $1\"\t\"$2\"\t\"$3\"\t{$fid}_Peak\"}' '{$sorted_file}' > '{$this->prefs->tmp_dir}/{$fname}.valid.bed'";
					...
					if ( $this->prefs->nohot === true ) {
						/* remove peaks overlapping with HOT regions
							HOT regions generated from from DataS8: http://www.modencode.org/publications/fly_2010pubs/index.shtml
							from the paper Identification of Functional Elements and Regulatory Circuits by Drosophila modENCODE Science 2010
							and includes all regions having a score of 8 or more
						*/
						$hot_bedfile = SYSTEM_DATA_ROOT . "/HOT_regions.dm3.bed";
	...
	private function prepare_files_old( $files_arr ) {

		if ( !is_array($this->files) )
			$this->files = array();

		$files_key = max( array_keys( $this->collections ) );
		foreach ( $files_arr['tmp_name']['file'] as $k => $tmp_loc ) {
			...
			$fname = $this->files[$files_key]['name'];
			$fid = $this->files[$files_key]['id'];

			$file_loc = $this->prefs->tmp_dir . "/" . $fname;

			/* cp PHP tmp file to tmp dir */
			exec( "cp \"{$tmp_loc}\" \"{$file_loc}\" " );

			/* convert Windows CRLF to LF */
/*
			$list = file_get_contents("{$this->prefs->tmp_dir}/{$fname}");
			$list = $this->normalise_linefeeds($list);
*/
			$file_list = file("{$this->prefs->tmp_dir}/{$fname}", FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
			$list = implode("\n", $file_list); //normalises line feeds  //file_get_contents( $oldfile );

			$fh = fopen("{$this->prefs->tmp_dir}/{$fname}", "w+");
			fwrite($fh, $list);
			fclose($fh);

/*
			$cmd = "cat {$fname} | awk '{ sub(/\r\$/,\"\"); print }' > {$fname}.tmp";
			exec( "cd {$this->prefs->tmp_dir} && $cmd ", $output, $retvar );

			if ( is_file("{$this->prefs->tmp_dir}/{$fname}.tmp") && filesize("{$this->prefs->tmp_dir}/{$fname}.tmp") > 0 ) {
				rename("{$this->prefs->tmp_dir}/{$fname}.tmp", "{$this->prefs->tmp_dir}/{$fname}");
			}
*/
			/* End convert Windows CRLF to LF */

			/* run bedtools sort */
			$cmd = BEDTOOLS . " sort -i \"{$fname}\" > \"{$fname}.sorted.bed\" 2>&1 ";

			exec( "cd {$this->prefs->tmp_dir} && $cmd ", $output, $retvar );

			if ( $retvar > 0 ) {
				$this->files[$files_key]['error'] = file_get_contents("{$this->prefs->tmp_dir}/{$fname}.sorted.bed");
				$this->error = true;
				continue;
			}

			/* the bed files have been verified and are trustworthy */

			if ( $this->prefs->unified_bed === true ) {
				//we need to make all peak names identical
				$command = "awk '{print $1\"\t\"$2\"\t\"$3\"\t{$fid}_Peak\"}' '{$fname}.sorted.bed' > '{$fname}.valid.bed'";
				exec( "cd {$this->prefs->tmp_dir} && {$command} ", $out, $retvar );
				if ( $retvar > 0 ) {
					$this->files[$files_key]['error'] = file_get_contents("{$this->prefs->tmp_dir}/{$fname}.valid.bed");
					$this->error = true;
					continue;
				}
			} else {
				...
				*/
				$genes = array();
				exec( "cd {$this->prefs->tmp_dir} && awk '{print \$4}' '{$fname}.sorted.bed' | sort | uniq ", $genes, $retvar );
				$out = count($genes);
				if ( !is_numeric($out) || $out < 1 ) {
					$this->files[$files_key]['error'] = "The supplied genomic regions BED file has no regions.";
					$this->error = true;
					continue;
				}
				if ( $out > GENE_BEDFILE_MAX_REGIONS ) {
					$this->files[$files_key]['error'] = "The supplied genomic regions BED file has {$out} regions, which is greater than the maximum allowed (".GENE_BEDFILE_MAX_REGIONS.").";
					$this->error = true;
					continue;
				}
				exec( "cd {$this->prefs->tmp_dir} && cp '{$fname}.sorted.bed' '{$fname}.valid.bed' ", $out, $retvar );
			...
			if ( $this->prefs->nohot === true ) {
				...
				*/
				$hot_bedfile = SYSTEM_DATA_ROOT . "/HOT_regions.dm3.bed";
				$cmd = BEDTOOLS . " subtract -A -a \"{$fname}.valid.bed\" -b \"{$hot_bedfile}\" > \"{$fname}.nohot.bed\" 2>&1 ";
				exec( "cd {$this->prefs->tmp_dir} && {$cmd}", $out, $retvar );
				if ( $retvar > 0 ) {
					$this->files[$files_key]['error'] = file_get_contents("{$this->prefs->tmp_dir}/{$fname}.nohot.bed");
					$this->error = true;
