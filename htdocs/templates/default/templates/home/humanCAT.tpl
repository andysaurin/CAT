...
{/if}
					...
					<div style="padding:20px;">
						...
						<div class="page-header">
							<h1 class="text">humanCAT
								<small>
									<i class="icon-double-angle-right"></i>
									The <b>human</b> ENCODE <b>C</b>hIP <b>A</b>ssociation <b>T</b>ester
								</small>
							</h1>

							<span class="row">
								<p>
									humanCAT allows you to easily compare your human (hg19) ChIP peaks from your peak-called BED file with those from most all human <a href="https://genome.ucsc.edu/ENCODE/dataMatrix/encodeChipMatrixHuman.html" title="ENCODE data grid" target="_blank">ENCODE ChIP peaks</a> from Tier 1 and Tier 2 cell lines.
								</p>
								<p>
							...
							<div class="row">
								<h4 class="text blue lead">To use humanCAT</h4>
								<ol>
									...
									<li><strong>Finally, you can simply obtain the association data selected, or upload your own <span class="red"><strong>hg19</strong></span>-mapped BED file.</strong></li>
								</ol>
							</div>
...
*}
																...
																<div class="form-group row">
																	<span class="col-xs-3">Omit certain datasets?</span>
																	<div class="col-xs-8">
																		<input type="text" name="omit_factors" id="form-field-tags" placeholder="ENCODE IDs ..." />

																	</div>
																	<div class="col-xs-1 no-padding-left">
																		<span  class="help-button param-help" style="display:none" data-rel="popover" data-trigger="hover" data-placement="left" title="Omit certain datasets." data-html="true" data-content="If you've previously run humanCAT and found that multiple ENCODE replicates are biasing the clustering analysis, you can omit replicates by entering their ENCODE IDs here.<br><br>The ENCODE IDs are given in parentheses next to the protein name in the output heatmap.<br>Enter an ID number followed by return for each dataset to omit from the results.">?</span>
																	</div>
																</div>
...
*/
								if ( $("#userBedType input:radio[name=analysisType]:checked").val() == "genechip" ) {
									...
									if ( $("#userBedType input:radio[name=analysisType]:checked").val() == "chipchip" && $(this).data('ace_input_files')[0] && $(this).data('ace_input_files')[0].size > 0 ) {
										...
										if ( !$("#" + $file_identifier).hasClass("has-success") ) {
											$("#" + $file_identifier).toggle(); //css("display","inline");
