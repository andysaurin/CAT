...
basedir <- getwd()
outfile <- paste(basedir, "DrosoCAT_heatmap.pdf", sep="/")

data <- read.table("DrosoCAT_scores.tsv", sep="\t", header=T )
...
bk <- c(
	seq(bk1, bk2, length=100),
	seq(bk3, bk4, length=100),
	seq(bk5, bk6, length=100),
	seq(bk7, bk8, length=100),
	seq(bk9, bk10, length=100),
	seq(bk11, bk12, length=100),
	seq(bk13, bk14, length=100)
)
hmcols <- colorRampPalette( rev(c(
	"#D41309",
	"#D73027",
